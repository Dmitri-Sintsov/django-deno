...
import { getBabelOutputPlugin } from "plugin-babel";
...
class InlineRollup {
    ...
    async generate(basedir: string | null, filename: string): Promise<Error | RollupOutput> {
        ...
        if (this.options.babel) {
            if (this.options.moduleFormat && ['es', 'module'].indexOf(this.options.moduleFormat) !== -1) {
                // https://docs.deno.com/runtime/reference/ts_config_migration/
                // https://www.npmjs.com/package/@rollup/plugin-babel
                // https://babel.dev/docs/babel-preset-env
                outputPlugins.push(
                    getBabelOutputPlugin({ presets: [babel_preset_env.default] })
                );
            } else {
                console.error(`babel is not suported for specified module format "${this.options.moduleFormat}"`);
            }
        }
